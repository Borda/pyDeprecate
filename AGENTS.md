...
- Use bare `import my_package` in runnable `.md` examples — `my_package` is not installed; use `from tests import collection_deprecate as my_package` (or `collection_misconfigured`, `collection_chains`) for executable examples. For CI-template snippets that intentionally show a placeholder, add `# phmdoctest:skip — CI template: replace my_package with your actual package` as the first line so phmdoctest skips execution
