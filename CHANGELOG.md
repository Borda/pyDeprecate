...
- **Cross-class method guard downgrades from `TypeError` to `UserWarning` (soft breaking change).** `@deprecated(target=OtherClass.method)` no longer raises `TypeError` at decoration time — it emits a `UserWarning` instead. Code that previously caught `TypeError` from this guard must migrate to `warnings.filterwarnings("error", message=".*cross-class method forwarding", category=UserWarning)` for equivalent strict behaviour. The guard heuristic uses `__qualname__` and may produce false positives for metaclass-generated or decorator-rewritten qualnames — downgrading to `UserWarning` allows those cases to be suppressed. **This is a breaking change for code that relied on `TypeError` being raised.** ([#166](https://github.com/Borda/pyDeprecate/pull/166))
