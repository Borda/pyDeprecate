...
- **`args_mapping` precedence: explicit new-name always wins when both old and new kwargs passed.** When a caller passed both the deprecated old argument name and the new name simultaneously (e.g. `fn(old_x=5, x=6)`), the remapped old-name value previously overwrote the explicit new-name value due to dict-comprehension last-write-wins ordering. The explicit new-name value now always wins, regardless of argument order at the call site. Affects `@deprecated` with `target=TargetMode.ARGS_REMAP` or a callable target, and `deprecated_class()` / `deprecated_instance()` with `args_mapping`. ([#198](https://github.com/Borda/pyDeprecate/pull/198))
