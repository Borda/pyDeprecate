...
- **`deprecated()` front door slimmed to the arguments common to both dispatch shapes** (`target`, `deprecated_in`, `remove_in`, `stream`, `num_warns`, `template_mgs`, `args_mapping`, `args_extra`, `skip_if`, `update_docstring`, `docstring_style`). The one shape-specific option, class-only `attrs_mapping`, now raises `TypeError` (unexpected keyword argument) on the front door — use `deprecated_class(attrs_mapping=...)` directly. `template_mgs` and `skip_if` passed through `@deprecated` on a class are now forwarded to the proxy (`template_mgs` was previously dropped silently on the class-dispatch path). ([#222](https://github.com/Borda/pyDeprecate/pull/222))
