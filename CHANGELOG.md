...
- **`pydeprecate` coding-agent plugin for Codex and Claude Code.** Ships shared `deprecate` and `remove` skills under `plugins/pydeprecate/`, with dual-host manifests and marketplace catalogs (`.claude-plugin/`, `.codex-plugin/`), separate from the `pyDeprecate` pip package. ([#232](https://github.com/Borda/pyDeprecate/pull/232))
