...
| `integration/`                | End-to-end tests that exercise the **public API** through the collection modules. Each test file covers one area of the public surface (functions, classes, audit, utils, docstrings, README examples). Tests here call `@deprecated`-decorated code exactly as a user would and assert on warnings, return values, and forwarded types. `test_readme.py` is generated by `phmdoctest` from README code blocks. |
| `unittests/`                  | Focused tests for **private/internal helpers** that are not directly exercised by integration tests. Each file mirrors a source module (`deprecation.py`, `audit.py`, `utils.py`). Tests import private symbols directly (e.g. `_raise_warn`, `_parse_version`) and use mocking/monkeypatching to keep each test isolated from external state.                                                                  |
