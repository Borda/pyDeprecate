...
| Command-line audit workflows | `pip install 'pyDeprecate[audit,cli]'` | Adds CLI dependencies (`fire`, `rich`) plus `packaging`-backed checks for `pydeprecate expiry`, `pydeprecate policy`, and `pydeprecate all` |
...
| `message_template` | `None`                | Custom deprecation message template (`%`-style placeholders)                                                                                                            |
