...
- 🛠️ Supports deprecating callables: functions, methods, class constructors, and **generator functions** (`def gen(): yield`) — warning fires eagerly at call time, before iteration begins; **async generators not supported in v0.9.0** (applying `@deprecated` to `async def` or `async def ... yield` creates a sync wrapper that silently breaks awaiting callers)
