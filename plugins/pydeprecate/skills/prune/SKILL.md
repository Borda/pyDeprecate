...
5. Keep only candidates whose `remove_in <= target_release` under PEP 440 ordering — the status table already applies it via installed audit support (`pyDeprecate[audit]`); never re-derive the set with lexical or float comparisons. Include exact-boundary deadlines; preserve future deadlines. A prerelease such as `2.0rc1` precedes `2.0`; use a final version only if the user actually requests final-release preparation. Missing/invalid deadlines remain unresolved; do not guess or rewrite them.
